...
%========================================
\subsection{例2：Set Cover（$O(\log n)$-近似）}
%========================================

\begin{blackboard}[Set Cover：具体例子]
...
\textbf{ILP}：
\begin{align*}
\min \quad & 3x_1 + 3x_2 + 3x_3 + 5x_4 \\
\text{s.t.} \quad & x_1 + x_4 \geq 1 \quad \text{(元素1)} \\
& x_1 + x_2 + x_4 \geq 1 \quad \text{(元素2)} \\
& x_1 + x_3 + x_4 \geq 1 \quad \text{(元素3)} \\
& x_2 + x_3 + x_4 \geq 1 \quad \text{(元素4)} \\
& x_2 + x_4 \geq 1 \quad \text{(元素5)} \\
& x_3 + x_4 \geq 1 \quad \text{(元素6)}
\end{align*}
...
\begin{blackboard}[Set Cover：随机舍入分析]

\textbf{LP 松弛}：$x_i \in [0, 1]$

\textbf{随机舍入}：以概率 $x_i^*$ 选择 $S_i$

\textbf{例子}：假设 LP 解 $x^* = (0.6, 0.5, 0.4, 0)$
...
\textbf{元素1只在 $S_1$ 中}：未覆盖概率 = $1 - 0.6 = 0.4$

\textbf{元素4在 $S_2, S_3$ 中}：未覆盖概率 = $(1-0.5)(1-0.4) = 0.3$

\textbf{重复 $k = O(\log n)$ 轮}，每个元素的未覆盖概率指数下降

\textbf{期望成本}：$O(\log n) \times \text{LP}$
...
$C_1 = T \vee T = T$ \checkmark

$C_2 = F \vee T = T$ \checkmark（$\bar{x}_1 = F$ 因为 $x_1 = T$）

$C_3 = T \vee F = T$ \checkmark

$C_4 = F$（$\bar{x}_2 = F$ 因为 $x_2 = T$）$\times$

满足 \textbf{3} 个子句。

\textbf{赋值2}：$x_1 = T, x_2 = F, x_3 = T$

$C_1 = T \vee F = T$ \checkmark

$C_2 = F \vee T = T$ \checkmark
...
$C_1 = F \vee T = T$ \checkmark

$C_2 = T \vee F = T$ \checkmark（$\bar{x}_1 = T$ 因为 $x_1 = F$）

$C_3 = T \vee T = T$ \checkmark
...
$C_2 = T \vee F = T$ \checkmark

$C_3 = F \vee T = T$ \checkmark

$C_4 = T$ \checkmark

满足 \textbf{3} 个子句。

\textbf{观察}：无论怎么赋值，最多只能满足3个子句。

\textbf{原因}：$C_1 = (x_1 \vee x_2)$ 和 $C_4 = (\bar{x}_2)$ 是``冲突''的。

如果 $x_2 = T$，$C_4$ 不满足；如果 $x_2 = F$，要满足 $C_1$ 就需要 $x_1 = T$，但这又影响 $C_2$...

\textbf{OPT = 3}（最优解满足3个子句）
...
$P[C_1 \text{ 不满足}] = P[x_1 = F] \times P[x_2 = F] = 0.5 \times 0.5 = 0.25$

$P[C_1 \text{ 满足}] = 1 - 0.25 = \mathbf{0.75}$

\textbf{一般规律}：子句有 $k$ 个文字（literal）

$P[\text{子句不满足}] = P[\text{所有文字都为假}] = (0.5)^k = \frac{1}{2^k}$

$P[\text{子句满足}] = 1 - \frac{1}{2^k}$
...
$C_1 = (x_1 \vee x_2)$：2个文字，$P[\text{满足}] = 0.75$

$C_2 = (\bar{x}_1 \vee x_3)$：2个文字，$P[\text{满足}] = 0.75$

$C_3 = (x_2 \vee \bar{x}_3)$：2个文字，$P[\text{满足}] = 0.75$

$C_4 = (\bar{x}_2)$：1个文字，$P[\text{满足}] = 0.5$

\textbf{期望满足的子句数}（用期望的线性性）：

$\mathbb{E}[\text{满足数}] = P[C_1\text{满足}] + P[C_2\text{满足}] + P[C_3\text{满足}] + P[C_4\text{满足}]$

$= 0.75 + 0.75 + 0.75 + 0.5 = \mathbf{2.75}$

\textbf{期望近似比} = $\frac{2.75}{3} \approx 0.917$

\textbf{一般保证}：每个子句至少有1个文字，满足概率 $\geq 0.5$

所以期望满足数 $\geq \frac{1}{2} \times$ 子句总数 $\geq \frac{1}{2} \times$ OPT
...
对每个变量 $x_i$，定义 $y_i \in [0, 1]$（表示 $x_i$ 为真的``程度''）

对每个子句 $C_j$，定义 $z_j \in [0, 1]$（表示 $C_j$ 被满足的``程度''）

\textbf{约束}（以 $C_2 = (\bar{x}_1 \vee x_3)$ 为例）：

$z_2 \leq (1 - y_1) + y_3$

（$C_2$ 满足需要 $\bar{x}_1$ 或 $x_3$ 为真，而 $\bar{x}_1$ 为真的程度是 $1 - y_1$）

\textbf{目标}：$\max \sum_j z_j$

\textbf{随机舍入}：对于 LP 解 $(y^*, z^*)$，以概率 $y_i^*$ 令 $x_i = \text{True}$

\textbf{分析}：这个方法对\textbf{长}子句好。

当子句有很多文字时，LP 舍入的满足概率接近 $1 - 1/e \approx 0.632$。
...
方法1（纯随机）对\textbf{短}子句好：$k=1$ 时概率 0.5，$k=2$ 时概率 0.75

方法2（LP舍入）对\textbf{长}子句好：$k$ 很大时概率接近 0.632

\textbf{组合方法}：以 50\% 概率用方法1，50\% 概率用方法2

\textbf{组合后的满足概率}：

$P[\text{组合满足}] = \frac{1}{2} P[\text{方法1满足}] + \frac{1}{2} P[\text{方法2满足}]$

\begin{center}
\begin{tabular}{c|cc|c}
$k$ & 方法1 & 方法2 & 组合 \\
\hline
1 & $0.5$ & $\approx 1.0$ & $\frac{0.5 + 1.0}{2} = \mathbf{0.75}$ \\
2 & $0.75$ & $\approx 0.75$ & $\frac{0.75 + 0.75}{2} = \mathbf{0.75}$ \\
3 & $0.875$ & $\approx 0.70$ & $\frac{0.875 + 0.70}{2} \approx 0.79$ \\
$\infty$ & $\to 1$ & $\to 0.632$ & $\to 0.82$
\end{tabular}
\end{center}
...
$x_{A1}^* = 0.5, x_{B1}^* = 0.5$（作业1各一半）

$x_{A2}^* = 0.5, x_{B2}^* = 0.5$（作业2各一半）

机器A负载 = $0.5 \times 1 + 0.5 \times 2 = 1.5$

机器B负载 = $0.5 \times 2 + 0.5 \times 1 = 1.5$
...
\begin{blackboard}[MAX-SAT：一个具体例子]

\textbf{问题}：3个变量 $x_1, x_2, x_3$，4个子句

$C_1 = x_1$（1个文字）

$C_2 = x_1 \lor x_2$（2个文字）

$C_3 = \neg x_1 \lor x_3$（2个文字）

$C_4 = x_1 \lor x_2 \lor x_3$（3个文字）

\textbf{最优解}：$x_1 = \text{True}, x_2 = \text{True}, x_3 = \text{True}$

$C_1$: True \checkmark，$C_2$: True \checkmark，$C_3$: True \checkmark，$C_4$: True \checkmark

\textbf{OPT = 4}（全部满足）

\textbf{问题}：随机算法能保证满足多少个？

\end{blackboard}

\begin{blackboard}[MAX-SAT：策略A（纯随机）]

\textbf{策略A}：每个变量独立以 $1/2$ 概率取 True

\textbf{分析每个子句}：

$C_1 = x_1$（1个文字）：$P[C_1 \text{ 满足}] = 1 - (1/2)^1 = \mathbf{1/2}$

$C_2 = x_1 \lor x_2$（2个文字）：$P = 1 - (1/2)^2 = \mathbf{3/4}$

$C_3 = \neg x_1 \lor x_3$（2个文字）：$P = 1 - (1/2)^2 = \mathbf{3/4}$

$C_4 = x_1 \lor x_2 \lor x_3$（3个文字）：$P = 1 - (1/2)^3 = \mathbf{7/8}$

\textbf{期望满足子句数}（期望线性性）：
\[
\mathbb{E}[Z] = \frac{1}{2} + \frac{3}{4} + \frac{3}{4} + \frac{7}{8} = \frac{4 + 6 + 6 + 7}{8} = \frac{23}{8} = \mathbf{2.875}
\]

\textbf{近似比}：$2.875 / 4 = 71.9\%$

\textbf{问题}：$C_1$ 只有 50\% 概率满足，太低了！

\end{blackboard}

\begin{blackboard}[MAX-SAT：策略B（LP舍入）]

\textbf{LP松弛}：变量 $y_i \in [0,1]$，子句值 $z_j \in [0,1]$

\textbf{假设LP解}：$y_1^* = 0.8, y_2^* = 0.6, y_3^* = 0.7$

\textbf{策略B}：以 $y_i^*$ 为概率取 True

$C_1 = x_1$：$P = y_1^* = \mathbf{0.8}$（比策略A的0.5好！）

$C_2 = x_1 \lor x_2$：$P = 1 - (1-0.8)(1-0.6) = 1 - 0.08 = \mathbf{0.92}$

$C_3 = \neg x_1 \lor x_3$：$P = 1 - (1-(1-0.8))(1-0.7) = 1 - 0.8 \times 0.3 = \mathbf{0.76}$

$C_4 = x_1 \lor x_2 \lor x_3$：$P = 1 - 0.2 \times 0.4 \times 0.3 = 1 - 0.024 = \mathbf{0.976}$

\textbf{期望}：$0.8 + 0.92 + 0.76 + 0.976 = \mathbf{3.456}$

\textbf{近似比}：$3.456 / 4 = 86.4\%$（比策略A好！）

\end{blackboard}

\begin{blackboard}[MAX-SAT：混合策略]

\textbf{混合}：以 $1/2$ 概率用策略A，$1/2$ 概率用策略B

\textbf{每个子句的满足概率}：

$C_1$：$P = \frac{1}{2} \times 0.5 + \frac{1}{2} \times 0.8 = 0.25 + 0.4 = \mathbf{0.65}$

$C_2$：$P = \frac{1}{2} \times 0.75 + \frac{1}{2} \times 0.92 = 0.375 + 0.46 = \mathbf{0.835}$

$C_3$：$P = \frac{1}{2} \times 0.75 + \frac{1}{2} \times 0.76 = 0.375 + 0.38 = \mathbf{0.755}$

$C_4$：$P = \frac{1}{2} \times 0.875 + \frac{1}{2} \times 0.976 = 0.4375 + 0.488 = \mathbf{0.9255}$

\textbf{期望}：$0.65 + 0.835 + 0.755 + 0.9255 = \mathbf{3.1655}$

\textbf{近似比}：$3.1655 / 4 = 79.1\%$

\textbf{理论保证}：$\geq 3/4 = 75\%$ \checkmark

\end{blackboard}

\begin{blackboard}[MAX-SAT：为什么混合能保证 3/4？]

\textbf{关键}：两种策略互补

\begin{center}
\begin{tabular}{c|c|c|c}
子句文字数 $k$ & 策略A概率 & 策略B概率 & 混合概率 \\
\hline
1 & $1/2 = 50\%$ & $\geq z^*$（可达100\%） & $\geq 3/4 \cdot z^*$ \\
2 & $3/4 = 75\%$ & $\geq 3/4 \cdot z^*$ & $\geq 3/4 \cdot z^*$ \\
3 & $7/8 = 87.5\%$ & $\geq 0.70 \cdot z^*$ & $\geq 3/4 \cdot z^*$ \\
$\infty$ & $\to 100\%$ & $\to 63.2\% \cdot z^*$ & $\geq 3/4 \cdot z^*$
\end{tabular}
\end{center}

\textbf{策略A}：$k$ 大时好（$1-(1/2)^k \to 1$）

\textbf{策略B}：$k$ 小时好（LP约束紧）

\textbf{混合}：取长补短，对所有 $k$ 都能保证 $\geq 3/4 \cdot z^*$

\end{blackboard}

\begin{blackboard}[MAX-SAT：完整证明框架]

\textbf{定义}：$I_j = 1$ 如果子句 $C_j$ 被满足

\textbf{期望线性性}（\textbf{即使 $I_j$ 不独立}！）：
\begin{align*}
\mathbb{E}[\text{满足数}] &= \mathbb{E}\left[\sum_j I_j\right] = \sum_j \mathbb{E}[I_j] = \sum_j P[C_j \text{ 满足}]
\end{align*}

\textbf{代入我们的例子}：
\[
\mathbb{E}[Z] = 0.65 + 0.835 + 0.755 + 0.9255 = 3.1655
\]

\textbf{检验}：$3.1655 \geq \frac{3}{4} \times 4 = 3$ \checkmark

\textbf{结论}：
\[
\boxed{\mathbb{E}[\text{满足子句数}] \geq \frac{3}{4} \cdot \text{OPT}}
\]

\end{blackboard}

\begin{blackboard}[Set Cover：一个具体例子]

\textbf{问题}：6个元素 $\{1,2,3,4,5,6\}$，3个集合
...
\textbf{LP解}：$x_1^* = x_2^* = x_3^* = 0.5$（每个集合选``半个''）

\textbf{验证LP可行性}：

元素1在 $S_1, S_3$：$x_1^* + x_3^* = 0.5 + 0.5 = 1 \geq 1$ \checkmark

元素3在 $S_1, S_2$：$x_1^* + x_2^* = 0.5 + 0.5 = 1 \geq 1$ \checkmark

元素5在 $S_2, S_3$：$x_2^* + x_3^* = 0.5 + 0.5 = 1 \geq 1$ \checkmark

\textbf{LP成本}：$2 \times 0.5 + 2 \times 0.5 + 2 \times 0.5 = 3$

\textbf{ILP最优}：必须选至少2个集合，成本 = 4

\end{blackboard}

\begin{blackboard}[Set Cover：朴素舍入的问题]

\textbf{朴素舍入}：以 $x_S^* = 0.5$ 的概率选每个集合

\textbf{分析元素1}：元素1在 $S_1$ 和 $S_3$ 中

$P[\text{元素1未覆盖}] = P[S_1 \text{未选}] \times P[S_3 \text{未选}]$

$= (1 - 0.5) \times (1 - 0.5) = 0.5 \times 0.5 = \mathbf{0.25}$

\textbf{类似地}：

元素2只在 $S_1$：$P[\text{未覆盖}] = 0.5$（很高！）

元素4只在 $S_2$：$P[\text{未覆盖}] = 0.5$

元素6只在 $S_3$：$P[\text{未覆盖}] = 0.5$

\textbf{Union Bound}：
\[
P[\text{存在未覆盖}] \leq 0.25 + 0.5 + 0.25 + 0.5 + 0.25 + 0.5 = \mathbf{2.25}
\]
...
\begin{blackboard}[Set Cover：关键不等式 $1-x \leq e^{-x}$]
...
\begin{center}
\begin{tabular}{c|c|c|c}
$x$ & $1-x$ & $e^{-x}$ & $1-x \leq e^{-x}$? \\
\hline
0.1 & 0.90 & 0.905 & \checkmark \\
0.3 & 0.70 & 0.741 & \checkmark \\
0.5 & 0.50 & 0.607 & \checkmark \\
0.7 & 0.30 & 0.497 & \checkmark \\
1.0 & 0.00 & 0.368 & \checkmark
\end{tabular}
\end{center}

\textbf{应用}：连乘变连加
\[
\prod_i (1 - x_i) \leq \prod_i e^{-x_i} = e^{-\sum_i x_i}
\]

\textbf{例子}：$(1-0.3)(1-0.4)(1-0.3) = 0.7 \times 0.6 \times 0.7 = 0.294$

$e^{-(0.3+0.4+0.3)} = e^{-1} = 0.368$

确实 $0.294 \leq 0.368$ \checkmark

\end{blackboard}

\begin{blackboard}[Set Cover：放大概率的数值例子]

\textbf{问题}：$n = 1000$ 个元素，朴素舍入未覆盖概率 $\approx 36.8\%$

\textbf{解决}：放大概率 $c \ln n$ 倍

取 $c = 2$，$\ln 1000 \approx 6.9$，放大倍数 $\approx 13.8$

\textbf{新策略}：以 $\min(1, 13.8 \times x_S^*)$ 的概率选 $S$

\textbf{元素未覆盖概率}：
\[
P[\text{未覆盖}] \leq e^{-13.8 \times 1} = e^{-13.8} \approx 10^{-6}
\]

\textbf{Union Bound}：
\[
P[\text{存在未覆盖}] \leq 1000 \times 10^{-6} = \mathbf{0.001}
\]

\textbf{结论}：以 99.9\% 概率，所有元素都被覆盖！

\textbf{代价}：成本放大 $13.8$ 倍，即 $O(\log n)$ 倍

\end{blackboard}

\begin{blackboard}[通用框架：随机舍入 + 贪婪修复]

\textbf{问题}：不想放大概率（成本太高），怎么办？

\textbf{随机舍入 + 贪婪修复}：

\begin{enumerate}
    \item \textbf{随机舍入}：以 $x_S^*$ 为概率选（不放大）
    \item \textbf{检查}：哪些元素未被覆盖？
    \item \textbf{贪婪修复}：对未覆盖元素，选性价比最高的集合
\end{enumerate}

\textbf{回到我们的例子}：

Step 1：假设随机舍入选了 $S_1$（概率0.5），没选 $S_2, S_3$

Step 2：未覆盖元素 = $\{4, 5, 6\}$

Step 3：$S_2$ 覆盖 $\{4, 5\}$，$S_3$ 覆盖 $\{5, 6\}$

选 $S_2$（覆盖2个），再选 $S_3$（覆盖剩下的6）

\textbf{总成本}：$2 + 2 + 2 = 6$（ILP最优是4，近似比1.5）

\end{blackboard}

\begin{blackboard}[贪婪修复：详细步骤]

\textbf{继续例子}：随机舍入后选了 $\{S_1\}$，未覆盖 $U = \{4, 5, 6\}$

\textbf{贪婪选择}：选``性价比''最高的集合
\[
\text{性价比} = \frac{|S \cap U|}{\text{cost}(S)}
\]

$S_2$：覆盖 $\{4, 5\} \cap U = \{4, 5\}$，性价比 = $2/2 = 1$

$S_3$：覆盖 $\{5, 6, 1\} \cap U = \{5, 6\}$，性价比 = $2/2 = 1$

\textbf{选 $S_2$}：$U = \{6\}$

\textbf{再选 $S_3$}：$U = \emptyset$

\textbf{分析}：

随机舍入期望成本 = $\sum x_S^* \cdot \text{cost}(S) = 3$

贪婪修复期望成本 = $O(\log |U|) \cdot \text{OPT}$（贪婪算法的保证）

\textbf{总成本}：$O(\log n) \cdot \text{OPT}$

\end{blackboard}

\begin{blackboard}[迭代松弛-舍入：Vertex Cover 例子]

\textbf{问题}：三角形图，3个顶点 A, B, C

\textbf{LP解}：$x_A^* = x_B^* = x_C^* = 0.5$

\textbf{迭代舍入}：

\textbf{第1轮}：所有变量都是0.5，没有``几乎整数''的

怎么办？利用LP结构！

\textbf{关键定理}：对于 Vertex Cover，LP 极点解中至少有一个变量是 0 或 1

\textbf{重新求解}：添加约束 $x_A + x_B + x_C \leq 2$（最多选2个）

新LP解：$x_A^* = 1, x_B^* = 0.5, x_C^* = 0.5$（或其他排列）

\textbf{第2轮}：$x_A^* = 1$，固定 $x_A = 1$

剩余问题：边 (B,C)，约束 $x_B + x_C \geq 1$

LP解：$x_B^* = x_C^* = 0.5$

\textbf{第3轮}：随机选一个，比如 $x_B = 1$

\textbf{结果}：选 $\{A, B\}$，成本 = 2（正好是最优！）

\end{blackboard}

\begin{blackboard}[Feasibility Pump：具体例子]

\textbf{问题}：$\min x_1 + x_2$ s.t. $x_1 + x_2 \geq 1.5$，$x_1, x_2 \in \{0,1\}$

\textbf{ILP最优}：$x_1 = x_2 = 1$，目标 = 2

\textbf{Feasibility Pump 迭代}：

\textbf{第1轮}：

LP解：$x_1^* = 0.75, x_2^* = 0.75$（刚好满足 $1.5$）

舍入：$\tilde{x}_1 = 1, \tilde{x}_2 = 1$

检查：$1 + 1 = 2 \geq 1.5$ \checkmark，\textbf{可行！返回}

\end{blackboard}

\begin{blackboard}[Feasibility Pump：需要多轮的例子]

\textbf{问题}：$\min x_1 + x_2 + x_3$

约束：$x_1 + x_2 \geq 1.5$，$x_1 + x_3 \geq 1.5$，$x_2 + x_3 \leq 1$

\textbf{第1轮}：

LP解：$x^* = (1, 0.5, 0.5)$（满足所有约束）

舍入：$\tilde{x} = (1, 1, 1)$（四舍五入）

检查：$x_2 + x_3 = 2 > 1$，\textbf{不可行}

\textbf{第2轮}：修改目标函数

新目标：$\min |x_1 - 1| + |x_2 - 1| + |x_3 - 1|$（让LP解靠近 $\tilde{x}$）

新LP解：$x^* = (1, 1, 0)$

舍入：$\tilde{x} = (1, 1, 0)$

检查：$1 + 1 \geq 1.5$ \checkmark，$1 + 0 = 1 < 1.5$，\textbf{不可行}
...
检查：$1+0=1 < 1.5$，$1+1=2 \geq 1.5$，$0+1=1 \leq 1$

还是不可行！但 ILP 最优是 $(1, 1, 0)$ 不行，实际是 $(1, 0.5, 0.5)$...

\textbf{这个问题的ILP最优}：$(1, 1, 0)$ 满足前两个但不满足第三个

实际上这个ILP\textbf{无解}！Feasibility Pump会检测到

\end{blackboard}

\begin{blackboard}[RINS：具体例子]

\textbf{场景}：已有一个可行解 $x^{\text{best}} = (1, 0, 1, 0, 1)$

当前 LP 解 $x^{\text{LP}} = (0.9, 0.1, 0.8, 0.3, 0.95)$

\textbf{Step 1：找``一致''变量}

$x_1$：$x^{\text{best}}_1 = 1$，$\text{round}(0.9) = 1$，\textbf{一致}

$x_2$：$x^{\text{best}}_2 = 0$，$\text{round}(0.1) = 0$，\textbf{一致}

$x_3$：$x^{\text{best}}_3 = 1$，$\text{round}(0.8) = 1$，\textbf{一致}

$x_4$：$x^{\text{best}}_4 = 0$，$\text{round}(0.3) = 0$，\textbf{一致}

$x_5$：$x^{\text{best}}_5 = 1$，$\text{round}(0.95) = 1$，\textbf{一致}

\textbf{Step 2}：所有变量都一致！

这说明当前解可能已经很好，或者需要扰动探索

\textbf{如果 $x_4^{\text{LP}} = 0.6$}：$\text{round}(0.6) = 1 \neq 0$，不一致

固定 $x_1, x_2, x_3, x_5$，只对 $x_4$ 求解小 ILP
...
\begin{blackboard}[条件期望法：继续决定变量]

\textbf{已固定}：$x_1 = T$

\textbf{Step 2：决定 $x_2$}

$W(T, x_2 = T)$：$C_2$ 必满足，$C_4$ 必满足

$= 1 + 1 + 0.5 + 1 = 3.5$

$W(T, x_2 = F)$：$C_2$ 仍满足（因为$x_1=T$），$C_4$ 仍满足
...
\textbf{最终解}：$(T, T, T)$，满足全部4个子句！

\end{blackboard}

\begin{blackboard}[Integrality Gap：LP 松弛的极限]

\textbf{定义}：$\text{Gap} = \max_{\text{实例}} \frac{\text{OPT}_{\text{ILP}}}{\text{OPT}_{\text{LP}}}$

\textbf{含义}：\textbf{任何}基于该LP的舍入都无法突破Gap
...
问题 & Gap & 最佳舍入 \\
\hline
Vertex Cover & 2 & 2-近似（紧！） \\
Set Cover & $\Theta(\log n)$ & $O(\log n)$（紧！） \\
MAX-SAT & 4/3 & 3/4-近似 \\
Max-Cut & 2（LP） & 需要SDP
\end{tabular}
\end{center}
...
\begin{blackboard}[Integrality Gap：数值例子]

\textbf{例1：三角形 Vertex Cover}

LP最优：$x_A = x_B = x_C = 0.5$，值 = 1.5

ILP最优：必须选2个顶点，值 = 2

Gap = $2 / 1.5 = \mathbf{4/3} \approx 1.33$

\textbf{例2：$K_n$ 完全图 Vertex Cover}（$n$ 个顶点）

LP最优：每个顶点选 0.5，值 = $n/2$

ILP最优：必须选 $n-1$ 个顶点，值 = $n-1$

Gap = $(n-1) / (n/2) = 2(n-1)/n \to \mathbf{2}$（当 $n \to \infty$）

\textbf{例3：Set Cover Gap}

$n$ 个元素，$n$ 个集合，集合 $S_i = \{i\}$（单元素集合）

加一个大集合 $S_0 = \{1, 2, \ldots, n\}$，成本 = $\ln n$
